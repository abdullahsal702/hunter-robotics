...
class RobotFollower:
	def __init__(self):
		...
		self.min_area = 500 # Can be smaller
				...
				if M['m00'] > 0:
					...
					if not self.obstacle_detected:
						...
						if any(d < 0.3 for d in self.lidar_data if not math.isinf(d) and not math.isnan(d)):
							print("Moving backwards from obstacle")
	...
	def handle_target_loss(self, display_image):
		...
		if self.frames_without_target > self.max_frames_without_target:
			...
			self.last_error = 0
			cv2.putText(display_image, "Searching for target...", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
